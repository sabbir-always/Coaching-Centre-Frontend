...
import { GET_SINGLE_TEACHER, PUT_TEACHER } from "../../context/Route";
...
const UpdateTeacherComponet = () => {
    ...
    const { register, handleSubmit, reset, setValue, formState: { errors }, } = useForm({
        resolver: zodResolver(TeacherSchema), defaultValues: { first_name: "", last_name: "", phone: "", email: "", department_id: "", blood_group: "", religion: "" }
    });

    useEffect(() => {
        const fetchTeacher = async () => {
            try {
                const response = await axios.get(`${GET_SINGLE_TEACHER}${id}`);
                if (response.data && response.data.success) {
                    ...
                    updateDepartmentState({
                        options_value: { value: data.department_id, label: data.department_name }
                    });
                }
            } catch (error) {
    ...
    const onSubmit = async (data) => {
        setIsLoading(true);
        try {
            const response = await axios.put(`${PUT_TEACHER}${id}`, {
                first_name: data.first_name,
                last_name: data.last_name,
                phone: data.phone,
                email: data.email,
                ...
                <div className="row justify-content-center">
                    <div className="col-md-6">
                        <form onSubmit={handleSubmit(onSubmit)} className="shadow-sm bg-light p-3 pt-0">
                            <h4 className="form_heading py-4">Update Teacher</h4>
...
export default UpdateTeacherComponet