import { GET_SINGLE_ADMISSION, PUT_ADMISSION } from "../../context/Route";
...
const UpdateAdmissionComponent = () => {
    const { id } = useParams();
        ...
        const fetchAdmission = async () => {
            try {
                ...
                if (response.data && response.data.success) {
                    ...
                    updateSectionState({ options_value: { value: data.section_id, label: data.section_name } });
                    updateDepartmentState({ options_value: { value: data.department_id, label: data.department_name } });
                }
            } catch (error) {
    ...
    const onSubmit = async (data) => {
        setIsLoading(true);
        try {
            const response = await axios.put(`${PUT_ADMISSION}${id}`, {
                ...
                <div className="row justify-content-center">
                    <div className="col-md-8">
                        <form onSubmit={handleSubmit(onSubmit)} className="shadow-sm bg-light p-3 pt-0">
                            <h4 className="form_heading py-4">Update Student</h4>
