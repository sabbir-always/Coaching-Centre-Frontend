...
import { POST_DEPARTMENT } from "../../context/Route";
...
const CreateDepartmentComponent = () => {
  ...
  const { register, handleSubmit, formState: { errors }, } = useForm({ resolver: zodResolver(DepartmentSchema) });

  const onSubmit = async (data) => {
    setIsLoading(true);
    try {
      const response = await axios.post(POST_DEPARTMENT, { department_name: data.department_name });
        ...
        <div className="row justify-content-center">
          <div className="col-md-5">
            <form onSubmit={handleSubmit(onSubmit)} className="shadow-sm bg-light p-3 pt-0">
              <h4 className="form_heading py-4">Create Department</h4>
