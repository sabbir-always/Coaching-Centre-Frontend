...
import { GET_SINGLE_PAYMENT, PUT_PAYMENT } from "../../context/Route";
...
const UpdatePaymentComponent = () => {
    ...
    useEffect(() => {
        const fetchPayment = async () => {
            try {
                const response = await axios.get(`${GET_SINGLE_PAYMENT}${id}`);
                if (response.data && response.data.success) {
                    const data = response.data.payload
                    reset({
                        date_and_time: data.date_and_time.split("T")[0],
                        admission_id: data.student_id,
                        payment: data.payment,
                        payment_method: data.payment_method
                    });
                    updateAdmissionPaymentDueState({
                        options_value: { value: data.student_id, label: data.student }
                    });
                }
            } catch (error) {
    ...
    const onSubmit = async (data) => {
        setIsLoading(true);
        try {
            const response = await axios.put(`${PUT_PAYMENT}${id}`, {
                ...
                <div className="row justify-content-center">
                    <div className="col-md-6">
                        <form onSubmit={handleSubmit(onSubmit)} className="shadow-sm bg-light p-3 pt-0">
                            <h4 className="form_heading py-4">Update Payment</h4>
